        ...
        for b in range(0, 3):
            ...
            for i in range(len(data)):
                for j in range(len(data)):
                    if j > i:
                        data0.append(data[i])  # negative sample
                        data1.append(data[j])

            imgs0.extend(data0)
            imgs1.extend(data1)

    return np.asarray(imgs0, np.float32), np.asarray(imgs1, np.float32)
...
# cost = tf.reduce_mean(tf.maximum(0.0, 1 - fc)) + regularization_loss
...
for step in range(500):
    ...
    for start, end in zip(range(0, len(trX0), N_BATCH), range(N_BATCH, len(trX0) + N_BATCH, N_BATCH)):
        _, c = sess.run([train_step, cost],
                        feed_dict={X0: trX0[start:end], X1: trX1[start:end], keep_prob: 1.0})
        costs.append(c)
